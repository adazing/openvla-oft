...
from prismatic.vla.constants import set_constants, IGNORE_INDEX
from prismatic.vla.datasets import RLDSBatchTransform, RLDSDataset, PatchPolicyVLADataset, InfiniteDataLoader
...
# FinetuneConfig is now defined in configs/finetune.yaml (Hydra)
...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        actions_hidden_states = (
            ...
            .reshape(batch_size, C.NUM_ACTIONS_CHUNK * C.ACTION_DIM, -1)
            .to(torch.bfloat16)
        )  # (B, act_chunk_len, D)
...
def run_diffusion_sampling(
    ...
    noise = torch.randn(
        size=(batch_size, C.NUM_ACTIONS_CHUNK, C.ACTION_DIM),
        device=device_id,
    ...
    for t in action_head.module.noise_scheduler.timesteps:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            actions_hidden_states = text_hidden_states[current_action_mask | next_actions_mask].reshape(
                batch_size, C.NUM_ACTIONS_CHUNK * C.ACTION_DIM, -1
            )  # (B, act_chunk_len, D)
...
def finetune(cfg: DictConfig) -> None:
    ...
    # Set robot constants from dataset config (overrides auto-detected defaults)
    set_constants(
    ...
    dataset_label = cfg.dataset.get("dataset_name", cfg.dataset.get("dataset_class", "unknown"))
    print(f"Fine-tuning OpenVLA Model `{cfg.vla_path}` on `{dataset_label}`")
    ...
    print(
        "Detected constants:\n"
        f"\tC.NUM_ACTIONS_CHUNK: {C.NUM_ACTIONS_CHUNK}\n"
        f"\tC.ACTION_DIM: {C.ACTION_DIM}\n"
        f"\tC.PROPRIO_DIM: {C.PROPRIO_DIM}\n"
        f"\tC.ACTION_PROPRIO_NORMALIZATION_TYPE: {C.ACTION_PROPRIO_NORMALIZATION_TYPE}"
    )
    ...
    if cfg.use_l1_regression:
        action_head = init_module(
            ...
            cfg,
            device_id,
            {"input_dim": vla.module.llm_dim, "hidden_dim": vla.module.llm_dim, "action_dim": C.ACTION_DIM},
            to_bf16=True,
        )
    ...
    if cfg.dataset.type == "rlds":
        # --- RLDS dataset path (original behavior) ---
        ...
        # --- patch_policy TrajectoryDataset path ---
        ...
        # Map dataset class name to class
        DATASET_CLASS_MAP = {
        ...
        if cfg.dataset.get("subset_fraction") is not None:
            ds_kwargs["subset_fraction"] = cfg.dataset.subset_fraction
        if cfg.dataset.get("prefetch") is not None:
            ds_kwargs["prefetch"] = cfg.dataset.prefetch
        if cfg.dataset.get("onehot_goals") is not None:
        ...
        # Slice into windows and split train/val
        ...
        train_dataset = PatchPolicyVLADataset(
            trajectory_slicer_dataset=train_sliced,
            action_tokenizer=action_tokenizer,
        ...
        # Create DataLoader with InfiniteDataLoader wrapper (map-style datasets are finite)
        sampler = DistributedSampler(train_dataset) if dist.is_initialized() and dist.get_world_size() > 1 else None
        ...
        if cfg.use_val_set:
            val_dataset = PatchPolicyVLADataset(
                trajectory_slicer_dataset=val_sliced,
                action_tokenizer=action_tokenizer,
                base_tokenizer=processor.tokenizer,
                image_transform=processor.image_processor.apply_transform,
                prompt_builder_fn=PurePromptBuilder,
